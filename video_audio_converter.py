...
class VideoAudioConverter:
    ...
    def convert_video_format(self, input_file, output_file, target_format):
        ...
        try:
            if not MOVIEPY_AVAILABLE:
                raise Exception("MoviePy is required for video format conversion")

            self.logger.info(f"Converting video format: {input_file} -> {output_file}")
    ...
    def convert_video_to_gif(self, input_file, output_file, max_width=480, fps=10, duration=None):
        ...
        try:
            if not MOVIEPY_AVAILABLE:
                raise Exception("MoviePy is required for video to GIF conversion")

            self.logger.info(f"Converting video to GIF: {input_file} -> {output_file}")
    ...
    def convert_audio_format(self, input_file, output_file, target_format):
        ...
        try:
            if not MOVIEPY_AVAILABLE:
                raise Exception("MoviePy is required for audio format conversion")

            self.logger.info(f"Converting audio format: {input_file} -> {output_file}")
